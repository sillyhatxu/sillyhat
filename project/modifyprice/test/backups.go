...
type UpdateShopItemPriceEventBodyDTO struct {
...
type UpdateShopItemSizeGuideEventBodyDTO struct {

	CustomerId int64 `json:"customerId"`

	UpdateShopItemSizeGuide *UpdateShopItemSizeGuideMessageDTO `json:"updateShopItemSizeGuide"`
}
...
type UpdateShopItemSizeGuideMessageDTO struct {

	ShopItemId int64 `json:"shop_item_id"`

	SizeGuideTable *ShopItemSizeGuideTableMessageDTO `json:"size_guide_table"`

	SizeGuideDescription string `json:"sizeGuideDescription"`
}

type ShopItemSizeGuideTableMessageDTO struct {
//
...
func concumer(messageJson string){
	...
	if(event.EventName == "updateShopItemPrice"){
		var eventBody UpdateShopItemPriceEventBodyDTO
		json.Unmarshal([]byte(event.EventBody), &eventBody)

		log.Info(eventBody)
		concumerBody(eventBody)
	}else if(event.EventName == "updateShopItemPrice"){
		var eventBody UpdateShopItemPriceEventBodyDTO
		json.Unmarshal([]byte(event.EventBody), &eventBody)
...
func concumerBody(eventBody UpdateShopItemPriceEventBodyDTO){
	tx,_ := db.Begin()
	defer tx.Commit()
	...
	if count == 0 {
		...
		stm,_ := db.Prepare("UPDATE ocb_syncer.shop_item_temp SET current_price = ?,original_price = ?,currency = ?,status = ?,validate_status = ?,last_modified_by = ?,last_modified_date = sysdate() WHERE id = ?")
